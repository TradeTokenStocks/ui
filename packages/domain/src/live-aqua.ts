...
import type { ContractAddress, DeployedStockToken, LiveHackathonDeployment } from './deployment';
...
function checkedBps(value: number, name: string, maximumExclusive: number): bigint {
  if (!Number.isInteger(value) || value < 0 || value >= maximumExclusive) {
    throw new RangeError(`${name} must be an integer from 0 to ${maximumExclusive - 1}`);
  }
  return BigInt(value);
}
...
export function multiplierBounds(
  ...
  if (multiplierE18 <= 0n) throw new RangeError('multiplierE18 must be greater than zero');
...
export function usdAllocationToTokenUnits({
  ...
  if (priceUsdE6 <= 0n || multiplierE18 <= 0n) throw new RangeError('price and multiplier must be positive');
  const amountUsdE6 = parseDecimalUnits(amountUsd, 6);
  return amountUsdE6 * 10n ** BigInt(tokenDecimals) * 10n ** 18n /
    (priceUsdE6 * multiplierE18);
}

function asCall(call: { to: string; data: string; value: bigint }): EncodedCall {
  return {
...
export function buildLivePeggedPosition({
  ...
  if (tokenA.underlying !== tokenB.underlying) throw new Error('Pegged positions require the same underlying stock');
  if (tokenA.address === tokenB.address) throw new Error('Pegged positions require two distinct token contracts');
  if (reserveA <= 0n || reserveB <= 0n) throw new RangeError('Both reserves must be greater than zero');
  ...
  if (feeBps > 0) builder.flatFeeAmountInXD(instructions.fee.FlatFeeArgs.fromBps(feeBps));

  const program = builder
    .peggedSwapGrowPriceRange2D(curve)
    .salt({ salt })
    .build();
  const order = Order.new({ maker: new Address(maker), traits: MakerTraits.default(), program });
  ...
  const aqua = new AquaProtocolContract(new AquaAddress(deployment.contracts.aqua));
  ...
  return {
    ...
    strategyHash: AquaProtocolContract.calculateStrategyHash(aquaOrder).toString(),
...
export function buildSwapCall({
  ...
  const router = new SwapVMContract(new Address(deployment.contracts.aquaSwapVmRouter));
  return asCall(router.swap({
